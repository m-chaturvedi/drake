...
''')
    ...
    for name, _, comment in list(sorted(output, key=lambda x: (x[0], x[1]))):
        ...
        name_utf_8 = name
        comment_utf_8 = comment
        space_char = u"\n" if "\n".encode("utf-8") in comment.encode("utf-8") else " "
        print(str("{}".format(comment.encode("utf-8"))))
        #print('\nstatic const char *%s =%sR"doc(%s)doc";' % (name_utf_8, space_char, comment_utf_8))
        #print(u"\nstatic const char *{} ={}R\"doc({})doc\";".format(name_utf_8, space_char, comment_utf_8))
