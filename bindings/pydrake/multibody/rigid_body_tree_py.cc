...
PYBIND11_MODULE(rigid_body_tree, m) {
  ...
  py::enum_<FloatingBaseType>(m, "FloatingBaseType",
      D(drake, multibody, joints, FloatingBaseType))
    .value("kFixed", FloatingBaseType::kFixed,
      D(drake, multibody, joints, FloatingBaseType, kFixed))
  ...
  tree_cls
    ...
    }, D(RigidBodyTree, getRandomConfiguration))
  ...
  m.def("AddModelInstanceFromUrdfFile",
        ...
        D(drake, parsers, urdf, AddModelInstanceFromUrdfFile));
