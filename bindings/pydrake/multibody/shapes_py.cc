...
#define D(...) DOC(DrakeShapes, Shape, __VA_ARGS__)
...
PYBIND11_MODULE(shapes, m) {
  ...
  py::enum_<Shape>(m, "Shape")
    .value("UNKNOWN", Shape::UNKNOWN, D(UNKNOWN))
    .value("BOX", Shape::BOX, D(BOX))
    .value("SPHERE", Shape::SPHERE, D(SPHERE))
    .value("CYLINDER", Shape::CYLINDER, D(CYLINDER))
    .value("MESH", Shape::MESH, D(MESH))
    .value("MESH_POINTS", Shape::MESH_POINTS, D(MESH_POINTS))
    .value("CAPSULE", Shape::CAPSULE, D(CAPSULE));
