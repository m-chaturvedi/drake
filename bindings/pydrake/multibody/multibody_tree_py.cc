...
void init_module(py::module m) {
  ...
  m.def("world_index", &world_index, D(multibody, world_index));
  ...
  // Frames.
  {
    ...
    cls
        .def("name", &Class::name, D(multibody, Frame, name))
        .def("body", &Class::body, py_reference_internal,
             D(multibody, Frame, body));
  }

  {
    using Class = BodyFrame<T>;
    py::class_<Class, Frame<T>> cls(m, "BodyFrame", D(multibody, BodyFrame));
  ...
  {
    using Class = Body<T>;
    py::class_<Class> cls(m, "Body", D(multibody, Body));
    BindMultibodyTreeElementMixin(&cls);
    cls
        .def("name", &Class::name, D(multibody, Body, name))
        .def("body_frame", &Class::body_frame, py_reference_internal,
             D(multibody, Body, body_frame));
  }

  {
    using Class = RigidBody<T>;
    py::class_<Class, Body<T>> cls(m, "RigidBody", D(multibody, RigidBody));
  }
...
#pragma GCC diagnostic pop  // pop -Wdeprecated-declarations
...

  {
    ...
    cls
        .def(py::init<const string&, const Frame<T>&,
             ...
             py::arg("damping") = 0,
             D(multibody, RevoluteJoint, RevoluteJoint, 4))
        .def("get_angle", &Class::get_angle, py::arg("context"),
             D(multibody, RevoluteJoint, get_angle))
        .def("set_angle", &Class::set_angle, py::arg("context"),
             py::arg("angle"), D(multibody, RevoluteJoint, set_angle));
  }

  {
    ...
    cls
        ...
        D(multibody, WeldJoint, WeldJoint, 3));
  }
  ...
  // Actuators.
  {
    ...
    BindMultibodyTreeElementMixin(&cls);
    cls
        .def("name", &Class::name, D(multibody, JointActuator, name))
        .def("joint", &Class::joint, py_reference_internal,
             D(multibody, JointActuator, joint));
  }

  // Force Elements.
  {
    using Class = ForceElement<T>;
    py::class_<Class> cls(m, "ForceElement", D(multibody, ForceElement));
    BindMultibodyTreeElementMixin(&cls);
  }
...

  {
    ...
    cls
        ...
        .def("num_bodies", &Class::num_bodies,
             D(MultibodyPlant, num_bodies))
        .def("num_joints", &Class::num_joints, D(MultibodyPlant, num_joints))
        .def("num_actuators", &Class::num_actuators,
             D(MultibodyPlant, num_actuators))
        .def("num_model_instances", &Class::num_model_instances,
             D(MultibodyPlant, num_model_instances))
        .def("num_positions",
             overload_cast_explicit<int>(&Class::num_positions),
             D(MultibodyPlant, num_positions))
        .def("num_positions",
             overload_cast_explicit<int, ModelInstanceIndex>(
                &Class::num_positions),
             py::arg("model_instance"), D(MultibodyPlant, num_positions, 2))
        .def("num_velocities",
             overload_cast_explicit<int>(&Class::num_velocities),
             D(MultibodyPlant, num_velocities))
        .def("num_velocities",
             overload_cast_explicit<int, ModelInstanceIndex>(
                 &Class::num_velocities), D(MultibodyPlant, num_velocities, 2))
        .def("num_multibody_states", &Class::num_multibody_states,
             D(MultibodyPlant, num_multibody_states))
        .def("num_actuated_dofs",
             overload_cast_explicit<int>(&Class::num_actuated_dofs),
             D(MultibodyPlant, num_actuated_dofs));
    // Construction.
    cls
        .def("AddJoint",
             ...
             D(multibody, multibody, plant, MultibodyPlant, AddJoint))
        .def("WeldFrames", &Class::WeldFrames,
             py::arg("A"), py::arg("B"),
             py::arg("X_AB") = Isometry3<double>::Identity(),
             py_reference_internal, D(multibody, multibody, plant,
        ...
        .def("AddForceElement",
             ...
             D(multibody, multibody, plant, MultibodyPlant, AddForceElement));
    // Topology queries.
    cls
        .def("HasBodyNamed",
             overload_cast_explicit<bool, const string&>(&Class::HasBodyNamed),
             py::arg("name"), D(multibody, multibody, plant, MultibodyPlant,
                                HasBodyNamed))
        .def("HasJointNamed",
        ...
        .def("GetFrameByName",
             ...
             D(multibody, multibody, plant, MultibodyPlant, GetFrameByName))
        .def("GetFrameByName",
             overload_cast_explicit<const Frame<T>&, const string&,
                                    ModelInstanceIndex>(
                 &Class::GetFrameByName),
             py::arg("name"), py::arg("model_instance"), py_reference_internal,
             D(multibody, multibody, plant, MultibodyPlant, GetFrameByName, 2))
        .def("GetBodyByName",
             overload_cast_explicit<const Body<T>&, const string&>(
                &Class::GetBodyByName),
             py::arg("name"), py_reference_internal,
             D(multibody, multibody, plant, MultibodyPlant, GetBodyByName))
        .def("GetBodyByName",
             overload_cast_explicit<const Body<T>&, const string&,
                                    ModelInstanceIndex>(
                &Class::GetBodyByName),
             py::arg("name"), py::arg("model_instance"), py_reference_internal,
            D(multibody, multibody, plant, MultibodyPlant, GetBodyByName, 2))
        .def("GetJointByName",
             [](const Class* self, const string& name) -> auto& {
               return self->GetJointByName(name);
             },
             py::arg("name"), py_reference_internal,
            D(multibody, multibody, plant, MultibodyPlant, GetJointByName))
        .def("GetJointActuatorByName",
             overload_cast_explicit<const JointActuator<T>&, const string&>(
                &Class::GetJointActuatorByName),
             py::arg("name"), py_reference_internal,
             D(multibody, multibody, plant, MultibodyPlant,
               GetJointActuatorByName));
    // Geometry.
    cls
        .def("get_source_id", &Class::get_source_id,
        D(multibody, multibody, plant, MultibodyPlant, get_source_id))
    ...
    cls
        .def("world_body", &Class::world_body, py_reference_internal,
             D(multibody, multibody, plant, MultibodyPlant, world_body))
        .def("world_frame", &Class::world_frame, py_reference_internal,
             D(multibody, multibody, plant, MultibodyPlant, world_frame))
        .def("tree", &Class::tree, py_reference_internal,
             D(multibody, multibody, plant, MultibodyPlant, tree))
        .def("is_finalized", &Class::is_finalized,
             D(multibody, multibody, plant, MultibodyPlant, is_finalized))
        .def("Finalize", py::overload_cast<SceneGraph<T>*>(&Class::Finalize),
             py::arg("scene_graph") = nullptr,
            D(multibody, multibody, plant, MultibodyPlant, Finalize));
    // Add deprecated methods.
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
...
void init_parsing(py::module m) {
  ...
  m.def("AddModelFromSdfFile",
        ...
        py::arg("scene_graph") = nullptr,
        D(multibody, parsing, AddModelFromSdfFile));
  m.def("AddModelFromSdfFile",
        py::overload_cast<
        ...
        py::arg("scene_graph") = nullptr,
        D(multibody, parsing, AddModelFromSdfFile, 2));
}
